...
static void createGridBg(SDL_Renderer* renderer) {
    ...
    int w = TILE_SIZE * GRID_SIZE + GAP * (GRID_SIZE + 1);
    int h = TILE_SIZE * GRID_SIZE + GAP * (GRID_SIZE + 1);
    gridBgTex = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, w, h);
    SDL_SetTextureBlendMode(gridBgTex, SDL_BLENDMODE_NONE);
    SDL_SetRenderTarget(renderer, gridBgTex);
    SDL_SetRenderDrawColor(renderer, 200, 190, 180, 255);
